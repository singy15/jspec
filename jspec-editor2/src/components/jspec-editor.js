...
import autoresizeEditor from "./autoresize-editor.js";
  ...
  methods: {
    ...
    updated(newKey, oldKey) {
      if(Array.isArray(this.node)) {
        ...
        if(newKey !== "") {
          if(newKey.indexOf(".") >= 0) {
            let insertIndex = Math.floor(parseFloat(newKey,10));
            let originIndex = parseInt(oldKey,10);

            if(insertIndex < originIndex) {
              insertIndex++;
            }

            newary.splice(insertIndex,0,tmp);
          } else {
